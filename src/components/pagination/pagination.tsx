...
export const Pagination = ({ totalCount, currentPage = 1, maxSelected = 10, onPageChange }: Props) => {
  const lastPage = Math.floor(totalCount / maxSelected)
  const previousPages = currentPage > 1 
    ? generatePagesArray({ from: currentPage - 1 - siblingsCount, to: currentPage - 1 }) 
    : []

  const nextPages = currentPage < lastPage 
    ? generatePagesArray({ from: currentPage, to: Math.min(currentPage + siblingsCount, lastPage) }) 
    : []
  return(
    <Stack direction={['column', 'row']} mt='8' justify='space-between' align='center' spacing='6'>
      ...
      <Stack direction='row' spacing='2'>
        ...
        {previousPages.length > 0 && previousPages.map(prev => (
          <PageButton onPageChange={onPageChange} number={prev} key={prev}/>
        ))}
        <PageButton onPageChange={onPageChange} number={currentPage} isCurrent/>
        {nextPages.length > 0 && nextPages.map(next => (
